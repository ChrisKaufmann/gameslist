...
func TestRatings(t *testing.T) {
	...
	print("\tSet Ratings\n")
	err = mc1.SetRating(3)
	if err != nil {t.Errorf("mc1.SetRating %s", err)}
	mc1r2 := mc1.Rating()
	if mc1r2 != 3 {t.Errorf("mc1 Rating, expected 3 got %v",mc1r2)}
	err = mc2.SetRating(5)
	if err != nil {t.Errorf("mc2.SetRating %s", err)}
	mc2r2 := mc2.Rating()
	if mc2r2 != 5 {t.Errorf("mc1 Rating, expected 5 got %v",mc1r2)}

	//GetAllRatings and compare to what we know reality is
	print("\tGetAllRatings\n")
	rl := coll.GetRatings()
